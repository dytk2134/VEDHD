...
def search():
    if request.method == "POST":
        ...
        Human_o = len(request.form.getlist("Human_o"))
        #Mouse expression date
        Mouse_o = len(request.form.getlist("mouse_o"))
        #Zebrafish expression data
        Zebrafish_o = len(request.form.getlist("zebrafish_o"))

        Annotation = request.form.get('data_Annotation')+"_"+request.form.get('ref')
        FinalResult_num = 0
        FinalResult_title =["user_input","species","gene_name","Ensembl_id","ref","chr","gene_start","gene_end","strand","gene_description"]
        if request.form.get('SearchType') == "Symbol":
            ...
            Ensembl_Gene = Ensembl_Gene % (Annotation,in_p)
